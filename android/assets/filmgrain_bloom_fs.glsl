...
void main()
{
	float bloom = 1.5 * texture2D (u_texture, v_texCoords).r;
	...
	float vignette = v_color.b*texture2D(u_borderTexture,v_texCoords).a;
	gl_FragColor = vec4(vec3(bloom*(1.0-grain) + grain), max(vignette*u_flicker + grain + scanLine, bloom));
}